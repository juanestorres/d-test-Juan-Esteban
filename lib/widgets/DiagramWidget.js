...
export class DiagramWidget extends React.Component {
  ...
  onMouseMove(event) {
    ...
    if (action instanceof SelectingAction && multiselectDrag) {
      ...
      //Checking mouse movement for discret movement. Juanes
      ...
      action.selectionModels.forEach(model => {
        if (model.model instanceof NodeModel || model.model instanceof PointModel) {
          //Set the grid "cell" size in pixels. Wasn't really sure if it was 25. Juanes
          let cellSize = 25;
          let xMovement = (
          ...
          //Set how many pixels should the node move to match the cell start. Juanes
          let xMovementAdjust = ( xMovement % cellSize );
          let yMovementAdjust = ( yMovement % cellSize );
          //Calculate final position. Juanes
          model.model.x = model.initialX + xMovement - xMovementAdjust;
          model.model.y = model.initialY + yMovement - yMovementAdjust;
        }
      });
